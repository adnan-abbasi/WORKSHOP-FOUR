...
namespace TravelExpertsAdmin
{
    ...
    public static class Validator
    {
        ...
        public static bool IsDecimal(TextBox textBox)
        {
            ...
            {
                MessageBox.Show(textBox.Tag + " must be a decimal  or whole number.", Title);
                textBox.Focus();
                return false;
            }
        }
        ...
        public static bool IsInt32(TextBox textBox)
        {
            ...
            {
                MessageBox.Show(textBox.Tag + " must be an integer or whole number .", Title);
                textBox.Focus();
                return false;
            }
        }
        // Date validator Rejex.

        public static bool IsvalidDate(TextBox tb)
        {
           
                Regex pattren = new Regex("[12]\\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[12]\\d|3[01])");

            if (!pattren.IsMatch(tb.Text))
            {
                MessageBox.Show("Date must have a Format YYYY-MM-DD");
                tb.SelectAll();
                tb.Focus();
                return false;
            }
